<template>
  <div>
    <v-container fluid>
      <v-row align="center">
        <v-col v-for="item in albums" :key="item.id" md="2" xs="6" cols="6" sm="3">
      ...
      <div v-if="contentOver || albums.length<8" class="text-center overline">No more content to view</div>
      <div
    ...
    getData: async function() {
      ...
      let body = {
        query: `mutation($index: Int!, $num: Int!){
                albumsListView(index: $index num: $num filter: {user: true}){
                    id
                    name
                    imageURL
                    likes
